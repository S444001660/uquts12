...
import '../models/device_model.dart'; // <-- أصبح هذا الاستيراد مستخدمًا الآن
...
class _LabsListScreenState extends State<LabsListScreen> {
  ...
  Map<String, int> _labDeviceCounts = {}; // <-- متغير جديد لتخزين عدد الأجهزة
  ...
  /// *** [تم التحديث] *** دالة لتحميل بيانات المعامل وعدد الأجهزة المرتبطة بها.
  Future<void> _loadLabs() async {
    try {
      setState(() => _isLoading = true);
      // جلب البيانات بشكل متوازٍ لتحسين الأداء
      final results = await Future.wait([
        FirebaseDatabaseService.getLabs(),
        FirebaseDatabaseService.getDevices(),
      ]);

      final labs = results[0] as List<LabModel>;
      ...
      // فرز القائمة حسب تاريخ الإنشاء (الأحدث أولاً)
  ...
  /// *** [جديد] *** دالة مساعدة لحساب عدد الأجهزة لكل معمل.
  ...
  /// *** [تم التحديث] *** ويدجت لبناء بطاقة المعمل مع عرض عدد الأجهزة.
  ...
  /// *** [جديد] *** ويدجت لبناء شارة منسقة لعرض عدد الأجهزة.
